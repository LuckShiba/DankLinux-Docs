---
title: Getting Started
description: Get started with Dank Linux installation and configuration
sidebar_position: 2
image: img/dankinstall.png
---

import Hero from '@site/src/components/Hero';
import ThemedImage from '@theme/ThemedImage';
import useBaseUrl from '@docusaurus/useBaseUrl';

<Hero
  asciiArt={`██████╗  █████╗ ███╗   ██╗██╗  ██╗    ██╗     ██╗███╗   ██╗██╗   ██╗██╗  ██╗
██╔══██╗██╔══██╗████╗  ██║██║ ██╔╝    ██║     ██║████╗  ██║██║   ██║╚██╗██╔╝
██║  ██║███████║██╔██╗ ██║█████╔╝     ██║     ██║██╔██╗ ██║██║   ██║ ╚███╔╝
██║  ██║██╔══██║██║╚██╗██║██╔═██╗     ██║     ██║██║╚██╗██║██║   ██║ ██╔██╗
██████╔╝██║  ██║██║ ╚████║██║  ██╗    ███████╗██║██║ ╚████║╚██████╔╝██╔╝ ██╗
╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝    ╚══════╝╚═╝╚═╝  ╚═══╝ ╚═════╝ ╚═╝  ╚═╝`}
  hideTitle={true}
/>

Welcome to Dank Linux! This guide will help you get started with installing and configuring the Dank Linux suite on your system.

## Quick Installation

The fastest way to get started is with our one-line installer:

```bash
curl -fsSL https://install.danklinux.com | sh
```

<figure style={{ textAlign: 'center' }}>
  <ThemedImage
    alt="DankInstall Installer"
    sources={{
      light: useBaseUrl('/img/dankinstalallight.png'),
      dark: useBaseUrl('/img/dankinstall.png'),
    }}
    style={{ maxWidth: '800px', width: '100%' }}
  />
  <figcaption>dankinstall interactive installer</figcaption>
</figure>

This will:
- Install all dependencies for your distribution
- Set up DankMaterialShell, dgop, dsearch, matugen, and other optional enrichments.
- Configure your chosen compositor (niri, Hyprland) and terminal (Ghostty, Kitty, Alacritty)

## Manual Installation

Manual installation is also supported, and simple. See the installation section for each component:

- [DankMaterialShell Installation](dankmaterialshell/installation/)
- [dgop Installation](dgop/installation/)
- [dsearch Installation](danksearch/installation/)

## Getting Help

- **Documentation**: Browse these docs for detailed guides
- **GitHub**: Report issues at [DankMaterialShell](https://github.com/AvengeMedia/DankMaterialShell/issues)
- **Discord**: `DankMaterialShell` has a subsection on the niri discord server, keep relevant discussions in the relevant `#dms-*` channels.
  - [niri discord](https://discord.gg/ppWTpKmPgT)

---

<p style={{fontSize: '0.875rem', color: 'var(--ifm-color-emphasis-600)'}}>
Looking for a pre-configured immutable OS? Check out <a href="https://github.com/zirconium-dev/zirconium/" style={{display: 'inline-flex', alignItems: 'center', gap: '0.25rem'}}><img src="/img/z.svg" alt="" style={{width: '1em', height: '1em', verticalAlign: 'middle'}} /> Zirconium</a>.
</p>