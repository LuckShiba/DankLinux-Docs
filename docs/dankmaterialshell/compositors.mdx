...
DankMaterialShell works with any Wayland compositor, but we provide optimized configurations for niri, Hyprland, Sway, dwl/MangoWC, and labwc. These configs include DMS-specific keybindings, proper layer shell rules, and recommended settings.
...
DMS will automatically apply key binds, gaps, window radius, and colors if you include the following files in your niri config.
...
If you used [dankinstall](../dankinstall), DMS is managed via systemd and starts automatically. Skip the `spawn-at-startup "dms" "run"` line - you'll have two instances otherwise.

niri has proper systemd session integration. To ensure DMS only runs under niri (useful if you also have Plasma/GNOME installed):
...
// Start the shell (skip if using systemd)
...
dankinstall configures these variables in `~/.config/environment.d/90-dms.conf`. You can skip this section unless you need to override specific values in your compositor config.
...
If you used [dankinstall](../dankinstall), DMS is managed via systemd. Skip the `exec-once = dms run` line.

Unlike niri, Hyprland doesn't have systemd session targets. If you use multiple desktop environments and only want DMS under Hyprland, disable the systemd unit and use compositor autostart instead:
...
dankinstall configures these in `~/.config/environment.d/90-dms.conf`. You can skip this section unless you need compositor-specific overrides.
