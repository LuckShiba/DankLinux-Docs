...
| Variant | Use Case |
|---------|----------|
| `.default` | Mode-aware colors (recommended for most uses) |
...
dankinstall sets `QT_QPA_PLATFORMTHEME=gtk3` in `~/.config/environment.d/90-dms.conf`. To switch to qt6ct, edit that file and change the value, then log out and back in.
...
If you used dankinstall, edit `~/.config/environment.d/90-dms.conf`:
```ini
...
**Activate:** Open command palette (`Ctrl+Shift+P`), select `Preferences: Color Theme`, choose `Dynamic Base16 DankShell`.
