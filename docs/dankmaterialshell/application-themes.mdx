...
sudo cp /usr/lib/systemd/user/dms.service ~/.config/systemd/user/dms.service
sudo chown $USER:$USER ~/.config/systemd/user/dms.service
...
**Install the theme (1.0.x):**

```bash
wget https://github.com/AvengeMedia/DankMaterialShell/raw/refs/tags/1.0.3/quickshell/matugen/dynamic-base16-dankshell.vsix
...
In DMS 1.2, the Ghostty configuration changes to:
