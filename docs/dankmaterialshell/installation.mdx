...
Hyprland doesn't initialize the systemd user session by default. If DMS doesn't start with systemd, you have two options:

**Option 1:** Use [uwsm](https://github.com/Vladimir-csp/uwsm) to manage your session.

**Option 2:** Create a session target manually:
...
Then add to your Hyprland config (after any `env =` lines):
...
This exports your environment to systemd and starts services bound to `graphical-session.target`.
